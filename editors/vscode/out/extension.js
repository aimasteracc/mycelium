...
const callGraphTree_1 = require("./callGraphTree");
function errMessage(err) {
    return err instanceof mycelium_sdk_1.MyceliumError ? err.message : String(err);
}
...
async function withClientAndEditor(action) {
...
async function runIndex(client) {
    return vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Mycelium: indexing workspace…" }, async () => {
 ...
 */
async function withFirstRunIndex(client, attempt) {
    try {
        ...
        const choice = await vscode.window.showInformationMessage("Mycelium hasn't indexed this workspace yet.", { modal: true }, "Index now");
        if (choice !== "Index now") {
            return undefined;
        }
...
async function copyContextForAI() {
    await withClientAndEditor(async (client, editor) => {
        ...
        if (selection.length > 0 && selection.length <= 400) {
            ...
            task = await (0, engine_1.resolveSymbolAtCursor)(client, editor);
            if (!task) {
        ...
        const ctx = await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: "Mycelium: gathering context…" }, () => withFirstRunIndex(client, () => client.context(resolvedTask, { maxNodes: 30 })));
        if (ctx === undefined) {
            return;
        }
        await vscode.env.clipboard.writeText(formatContextForAI(resolvedTask, ctx));
        vscode.window.showInformationMessage("Mycelium: context copied — paste it to your AI assistant. (May include source from your workspace.)");
    });
}
/** Shape the context as a ready-to-send prompt, not a raw data dump. */
function formatContextForAI(task, ctx) {
    return [
...
async function showEdges(kind) {
    await withClientAndEditor(async (client, editor) => {
        const path = await (0, engine_1.resolveSymbolAtCursor)(client, editor);
        if (!path) {
            return;
        }
        const result = (kind === "callers"
            ? await client.getCallers(path)
            : await client.getCallees(path));
...
async function showSymbolInfo(output) {
    await withClientAndEditor(async (client, editor) => {
        const path = await (0, engine_1.resolveSymbolAtCursor)(client, editor);
        if (!path) {
            return;
...
async function showCallGraph(tree) {
    await withClientAndEditor(async (client, editor) => {
        const path = await (0, engine_1.resolveSymbolAtCursor)(client, editor);
        if (path) {
...
async function revealSymbol(symbolPath) {
    ...
    try {
        const editor = await vscode.window.showTextDocument(await vscode.workspace.openTextDocument(vscode.Uri.file(resolved)));
        ...
        const lineRaw = (span.start_line ?? span.line);
...
function activate(context) {
    ...
    const callGraph = new callGraphTree_1.CallGraphTreeProvider();
    context.subscriptions.push(output, vscode.window.registerTreeDataProvider("mycelium.callGraph", callGraph), vscode.commands.registerCommand("mycelium.copyContextForAI", copyContextForAI), vscode.commands.registerCommand("mycelium.findCallers", () => showEdges("callers")), vscode.commands.registerCommand("mycelium.findCallees", () => showEdges("callees")), vscode.commands.registerCommand("mycelium.symbolInfo", () => showSymbolInfo(output)), vscode.commands.registerCommand("mycelium.index", () => withClient(async (client) => {
        vscode.window.showInformationMessage(`Mycelium: ${await runIndex(client)}`);
    })), vscode.commands.registerCommand("mycelium.showCallGraph", () => showCallGraph(callGraph)), vscode.commands.registerCommand("mycelium.revealSymbol", (p) => revealSymbol(p)));
    if (vscode.workspace.getConfiguration("mycelium").get("indexOnActivate")) {
