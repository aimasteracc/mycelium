...
pub fn index_path(root: &Path) -> Result<(Store, IndexStats)> {
        ...
        .filter(|e| e.file_type().is_some_and(|ft| ft.is_file()))
    {
        ...
        let extractor = match ext {
            ...
            "rs" => &rs_ext,
            "go" => &go_ext,
            // C++ primary extensions only; `.h` defaults to the C extractor.
...
mod tests {
    ...
    fn index_path_extracts_cpp_hpp_header() {
        ...
        let (store, stats) = index_path(tmp.path()).unwrap();
        assert_eq!(stats.files, 1, ".hpp should be indexed as C++");
        assert!(
    ...
    fn index_path_extracts_cpp_cc_extension() {
        ...
        let (store, stats) = index_path(tmp.path()).unwrap();
        assert_eq!(stats.files, 1, ".cc should be indexed as C++");
        assert!(
    ...
    fn index_path_extracts_csharp_method() {
        ...
        assert!(
            ...
            "C# method must be nested under its class"
        );
    }
