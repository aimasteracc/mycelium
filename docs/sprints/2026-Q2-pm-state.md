...
| Last updated | 2026-06-07 (PM dispatch v98 — PR #629 MERGED (`be7a330`, RFC-0116 Phase 1 AC-2 + AC close); PR #631 closed superseded; Codex P1 on #631 rejected (stale-SHA, CI DCO ✅); Codex P2 on #631 fixed in v98 (priority renumber)) |
| Current sprint | **v0.3.0 ceremony READY** (P0 — founder action) **+ RFC-0120 Phase 1 next** (P1 — autonomous). RFC-0115/0116/0117 Phase 1 COMPLETE on develop (PR #629 merged `be7a330`). RFC-0119 Phase 1+2 ✅. |
...
8. **RFC-0120 Phase 1** (Issue #614): Token-accounting module `measure_corpus` + committed corpus. Design on develop (`33125d5c`). **← next P1 autonomous task.**
...
13. **VS Code Phase 1.5**: `vsce publish` wiring + marketplace metadata (after v0.3.0 ships; founder sign-off).
14. **GitHub Action live run**: Test the action on the Mycelium repo itself with a real PR (after v0.3.0 ships).

**P2 — Deferred:**
15. **MCP god-file split slice 4** — lib.rs ~4,485 lines.
16. **RFC-0104 cold SLA numbers**: Charter §2 amendment (founder, after nightly data collected).
17. **Skills marketplace submission**: metadata sign-off (founder).
18. **Issue #555 auto-close**: Will close automatically when PR #568 merges to main.

---

## Dispatch state (2026-06-07 v98)
...
| PM | **DONE ✅** | v98: PR #629 MERGED (`be7a330`); PR #631 closed superseded (Codex P1 rejected stale-SHA, Codex P2 acknowledged + fixed numbering); PM state v98 written. |
...
| rust-implementer | **P1** | RFC-0120 Phase 1 (Issue #614): token-accounting `measure_corpus` + committed corpus. Design on develop (`33125d5c`). |
