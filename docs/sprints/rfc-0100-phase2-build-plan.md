...
**performance cliff**, and the on-disk adjacency violates the ADR-0009 sorted-Vec contract.
...
| 5 | **HIGH** | Adjacency stored **UNSORTED** (`fwd.push` / `rev.push`) — violates ADR-0009 §2 (sorted `Vec<u64>`). `outgoing()`/`incoming()` sort at read time to compensate, so on-disk bytes are non-canonical and `contains()` can't become a binary search. | 449, 454; read-sort 484, 490 |
...
**ADR-0009 is currently aspirational, not implemented:** §6 ("ONE redb WriteTransaction per file
...
The first P2-T05 implementation slice adds the ADR-0009 `file_index` table to
