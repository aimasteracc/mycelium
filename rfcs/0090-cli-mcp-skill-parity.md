...
- [ ] `skills/INDEX.md` is generated by a script and lists every
      (capability → covering skill) pair.
- [ ] `crates/mycelium-cli/src/parity.rs` (or equivalent CI script)
      walks the three surface inventories and enforces the five
      invariants.
- [ ] CHARTER.md §5.13 amended with the Three-Surface Rule.
- [ ] CLAUDE.md "Hard Rules" updated.
- [ ] ADR-0007 records the architectural decision.
- [ ] The `parity-backfill` audit is seeded with one issue per
      existing un-covered capability, grouped by proposed Skill
      category.
- [ ] PR template gains a "Three-Surface Self-Check" section.
- [ ] `.github/workflows/parity.yml` runs the parity check on every
