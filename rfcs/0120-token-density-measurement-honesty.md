...
- [ ] **RED-testable (Phase 3 cross-surface byte identity — NEW harness):** `crates/mycelium-cli/tests/cli_token_stats.rs` spawns the compiled `mycelium token-stats` binary, captures stdout, calls the MCP tool body in-process over the same `measure_corpus` core, and `assert_eq!`s the two JSON strings. (No `tests/contract.rs`-style harness exists for this — it is built here.)
- [ ] **RED-testable (Phase 3 exception retraction):** `scripts/check` asserts no tool has both a CLI twin AND an `EXCEPTION: MCP-only` line; specifically that `get_token_stats` no longer carries `EXCEPTION: MCP-only` at `skills/INDEX.md:122`, is reclassified as a full CLI↔MCP↔Skill triple, and the "parity-backfill epic (v0.1.4)" wording is dropped from `skills/index-management/SKILL.md:332`.
- [ ] **RED-testable (Skill coverage):** `scripts/check` confirms `mcp__mycelium__get_token_stats` (the exposed name) appears in ≥1 `SKILL.md` `allowed-tools`.
- [ ] **RED-testable (tool count unaffected):** `tests/contract.rs`'s `EXPECTED_TOOL_COUNT` (93) is unchanged by the Phase 3 body/description rewrite (no tool added/removed).
...
- **Corpus capture source corrected TWICE.** Reviewer-1 (blocking; REFUTED) killed the draft's `tests/contract.rs` "indexed source tree" (empty/unindexed, `contract.rs:9–11`). Then **Codex P2 (#611)** killed the first correction's `tests/e2e/fixtures/ripgrep/` — that path is *fetched* by `scripts/fetch-e2e-fixtures.sh` (`real_projects.rs:8–34`), not committed. Final resolution: **commit the captured corpus JSON** (CI reads it, no fixture/network at test time); regeneration uses a small committed self-fixture or the fetched ripgrep guarded by an existence check.
