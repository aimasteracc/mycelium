...
- [ ] `Store::batch_reachable_to(ids, kind, max_depth)` returns union of individual `reachable_to` results.
...
- [ ] `mycelium_batch_reachable_to`: valid request returns `{ reachable, count }`.
- [ ] `mycelium_batch_reachable_to`: unknown edge_kind returns `{ error }`.
- [ ] `mycelium_batch_reachable_to`: empty paths returns empty result.
- [ ] `mycelium_batch_reachable_to`: unknown paths silently skipped.
