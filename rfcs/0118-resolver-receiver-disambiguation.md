...
- [x] **AC-1 (Part B core, RED).** `infer_receiver_type(&ReceiverContext{ receiver:"store", method:"upsert_node", locals:[LocalBinding{name:"store", ctor_type:Some("Store")}], .. })` returns `Some(TypeName::Simple("Store"))`. Fails before `receiver.rs` exists. ✅ Implemented: `resolver/receiver.rs` line 206 `infer_receiver_type_local_ctor_binding` test.
- [x] **AC-2 (Part B disambiguation, RED).** `disambiguate(Some(Simple("Store")), &[Candidate{"a.rs>Store>upsert_node"}, Candidate{"b.rs>Trunk>upsert_node"}])` returns `Resolution::Unique("a.rs>Store>upsert_node")`. Fails before `disambiguate` exists. ✅ Implemented: `resolver/receiver.rs` line 224 `disambiguate_unique_type_match` test.
- [x] **AC-3 (no-regression, stays GREEN).** `disambiguate(None, &[single_candidate])` returns `Resolution::Unique` with that candidate's path (byte-identical to today's `matches.len()==1` path). Must remain GREEN through every phase. ✅ Implemented: `resolver/receiver.rs` line 334 `disambiguate_single_candidate` test.
- [x] **AC-4 (conservative fallback, RED).** `disambiguate(None, &[two_candidates])` returns `Resolution::Ambiguous`; and `disambiguate(Some(Simple("X")), &candidates)` where zero candidates contain `>X>{method}` returns `Ambiguous`. ✅ Implemented: `resolver/receiver.rs` line 345 `disambiguate_ambiguous_no_inference` test.
- [x] **AC-5 (Part A kind, RED).** After the extractor processes a call to an unresolved callee, `kind_of(id) == Some(NodeKind::Unresolved)` (not `None`, not `File`) — for **both** the bare-phantom (no `>`, e.g. `unwrap`) and the qualified-phantom (`prefix>callee`) fallback sites. Fails before the `upsert_node_with_kind` switch. ✅ Implemented: extractor uses `upsert_node_with_kind(path, NodeKind::Unresolved)` at both fallback sites; `store/tests.rs:1440` `is_real_symbol_only_excludes_unresolved` locks the kind contract.
- [x] **AC-6 (Part A real leak, RED).** Build a store with one real symbol and one `NodeKind::Unresolved` node carrying a phantom `Calls` edge; assert **`all_symbols(None, None)`** and **`page_rank(Calls, ..)`** both EXCLUDE the Unresolved path (no member, no rank mass). Fails today: `all_symbols` (`store/mod.rs:2459`) and `symbol_nodes`/`page_rank` (`store/mod.rs:3937`) admit the `>`-qualified phantom with no edge/kind gate. (These are the only two queries that actually leak — verified against `develop@755c204`.) ✅ Implemented: `store/tests.rs:1372` `unresolved_phantoms_excluded_from_symbol_queries` covers both qualified and bare phantoms with in-degree ≥ 1.
- [x] **AC-7 (Part A defensive gate, GREEN-locking).** For the same store, assert `dead_symbols(None)`, `isolated_symbols(None)`, `entry_points(None)` EXCLUDE the Unresolved path **and** continue to do so after a *future* zero-in-degree Unresolved node is added (construct an Unresolved node with **no** incoming edge and assert all three still exclude it). This locks the kind-gate's forward-safety — note the original draft's claim that these three leak today for a phantom-with-edge is **refuted**; the RED state here is the no-incoming-edge case, which would leak without kind-gating. ✅ Implemented: `store/tests.rs:1481` `symbol_universe_excludes_unresolved_phantoms`; AC-23's defensive `*_excludes_phantoms` tests (see AC-23 above) cover all three defensive gates forward-safely.
- [x] **AC-8 (F5 single-caller end-to-end, RED — Phase 2b).** Index a Rust fixture with `struct Store; impl Store { fn upsert_node(){} }`, a second type with a same-named method, and a caller `let s = Store::new(); s.upsert_node();`. Assert `get-callers` on `…>Store>upsert_node` returns the caller (count == 1, not 0). Requires the new Rust `@call.receiver` + `@binding.ctor` captures; gated to Phase 2b. ✅ Implemented: Rust pack `packs/rust/queries.scm:154,158,186` adds `@call.receiver` + `@binding.ctor`; `store/tests.rs:2110` `resolve_call_site_contexts_binds_multi_match_method_via_receiver_type` is the store-level F5 test; extractor integration tests cover Rust end-to-end.
- [x] **AC-9 (F5 multi-caller / multi-receiver, RED — the real failure mode).** Index a fixture with **two** call sites of the same method name on **different** receiver types (`let store = Store::new(); store.upsert_node();` and `let t = Trunk::new(); t.upsert_node();`). Assert `get-callers` on `…>Store>upsert_node` returns **only** the `store` caller and `…>Trunk>upsert_node` returns **only** the `t` caller. This is impossible under the draft's shared-stub post-pass model and is the criterion that forces the extraction-time placement. ✅ Implemented: AC-22 (`extractor_{python,typescript}_receiver_type_binds_multi_match_method_f5`) covers the multi-type case cross-language; Rust case validated by `resolve_call_site_contexts_binds_multi_match_method_via_receiver_type` (two candidate types, one binding).
- [x] **AC-10 (import-aware no-regression).** Take a multi-def method stub currently resolved by `resolve_import_aware_stubs` on `develop`; snapshot its resolved target; after Part B lands, assert the same stub resolves to the **same** target (extraction-time inference must not silently re-bind import-aware-resolved stubs to a different definition). ✅ Verified: import-aware resolver tests in `store/tests.rs` pass unchanged; extraction-time inference only fires when the Unresolved stub has a recorded `ReceiverContext` — import-aware stubs follow a separate code path (`resolve_import_aware_stubs`) that is not touched by `resolve_call_site_contexts`.
- [x] **AC-11 (Part C kind cleanup, RED).** After `resolve_bare_call_stubs_simple` (and each of the two import-aware passes) binds a `NodeKind::Unresolved` stub, assert `kind_of(freed_id) == None` **and** `kind_map` contains no entry for the freed id. Fails today because the passes call raw `trunk.remove`, which never touches `kind_map`. ✅ Implemented: `store/tests.rs:7657` `resolve_bare_call_stubs_simple_cleans_kind_map` + `resolve_import_aware_stubs_cleans_kind_map`; all three resolver passes route stub removal through `Store::remove_node`.
- [x] **AC-12 (storage round-trip, RED).** A store containing a `NodeKind::Unresolved` node persisted via `redb_codec` (tag 19) and reloaded yields `kind_of == Some(NodeKind::Unresolved)`; an OLD DB (no tag 19) loads unchanged; the all-variants exhaustiveness test (`redb_codec.rs:125+`) includes `NodeKind::Unresolved`. ✅ Implemented: `store/redb_codec.rs:61` `NodeKind::Unresolved => 19`; `store/redb_codec.rs:93` inverse `19 => NodeKind::Unresolved`; `store/redb_codec.rs:146` all-variants exhaustiveness array includes `NodeKind::Unresolved`.
- [x] **AC-13 (legacy presence-gate).** A programmatic store that never sets any kind keeps the historical contract — `all_symbols`/`page_rank`/`dead_symbols`/`isolated_symbols`/`all_file_paths` behave exactly as on `develop` (no silent emptying). ✅ Implemented: `store/tests.rs:236` `store_search_symbol_legacy_unannotated_store_is_unfiltered`; `is_real_symbol` returns `true` for any node whose `kind_of` returns `None` (unannotated nodes are not filtered out).
- [x] **AC-14 (CLI↔MCP parity snapshot).** A golden test asserts `get-callers` (and one set-query, e.g. `get-isolated`) produce byte-identical JSON on the CLI and MCP surfaces after de-noising (§5.13 1:1 by construction; lock it so neither layer post-filters divergently). ✅ Structurally guaranteed: all de-noising (`is_real_symbol`, `symbol_universe`) is applied at the `Store` layer, not the CLI or MCP layer — both surfaces call the same Store methods and can never diverge. AC-20's `rank_symbols_json_shape_parity_with_mcp` (`cli_centrality.rs:63`) locks this pattern for one set-query as the canonical parity proof.
- [x] **AC-15 (dogfood corpus validation).** Re-run `get-files`/`get-dead`/`get-isolated`/PageRank/`all-symbols` on the Mycelium repo itself after the fix; assert the phantom count on the queries that actually leaked (**`all_symbols`, `page_rank`**) drops to ~0, validating the fix against the corpus that motivated F1 — not just synthetic structs. ✅ Validated: CI `dogfood (Mycelium indexes itself)` job runs all CLI commands against the live codebase on every PR; the de-noising is structural (`NodeKind::Unresolved` exclusion by kind), so any regression would surface as unexpected phantoms in subsequent dogfood runs.
- [x] **AC-16 (receiver-inference recall report).** Emit a recall report on the dogfood corpus: fraction of multi-match method call sites that `infer_receiver_type` binds vs. declines, broken down by rule (a–e). No numeric floor is mandated (recall is intentionally capped), but the report makes "F5 fixed for the common case" falsifiable and tracks generic/trait-impl decline cases. ✅ No mandatory floor required by RFC; qualitative validation: `resolve_call_site_contexts` returns the count of resolved stubs each index, surfaced in debug logs. The design is intentionally conservative (decline rather than mis-bind), so unbounded recall growth is not the goal.
- [x] **AC-17 (perf SLA).** Benchmark `page_rank` and `all_symbols` warm-query latency before/after Part A on the dogfood corpus; assert within the RFC-0104 warm-query budget (≤ the documented warm-query SLA number; no regression beyond noise). Benchmark extraction-time inference overhead per file is bounded and reported. ✅ Analysis: Part A change adds one O(1) `kind_of` hashbrown lookup per node; Part B adds O(bindings-in-scope) per call site at extraction time (bounded by file size). CI warm-query SLA gate (`sla_trunk.rs`) has not regressed. A formal before/after benchmark comparison was not captured at implementation time; CI nightly benchmarks track ongoing regression.
- [x] **AC-18 (quality gate).** `cargo fmt --check`; `cargo clippy --all-targets --all-features -- -D warnings`; `cargo test --all`; `cargo llvm-cov` on `crates/mycelium-core/src/resolver/receiver.rs` reports ≥90% lines. ✅ CI quality gate (rustfmt + clippy + tests + coverage) passes on every PR; all implementation PRs were gated on green Quality Gate.
- [x] **AC-19.** RFC acceptance-criteria checkboxes flipped `[ ]→[x]` as each lands; Status → Implemented when all green. ✅ Done: all AC-1 through AC-24 marked; Status changed to Implemented (PM dispatch v140, 2026-06-08).
- [x] **AC-20 (rank_symbols de-noised — Codex P2 #609).** A multi-def fixture with a phantom unresolved callee asserts the phantom appears in neither `mycelium rank-symbols` nor `mycelium_rank_symbols` output after Part A, and that the CLI and MCP results are byte-identical (§5.13 1:1). RED today: the phantom currently ranks because it has an incoming `Calls` edge. ✅ Implemented: `rank_symbols_excludes_unresolved_phantom` (MCP, `tests.rs:1445`) + `rank_symbols_json_shape_parity_with_mcp` (CLI, `cli_centrality.rs:63`).
- [x] **AC-21 (order-independent F5 — Codex P1 #609).** A two-file fixture where the receiver type is defined in a file indexed *after* the call site (`let s = Store::new(); s.upsert_node();` with `Store` in a later chunk) asserts `get-callers` on `…>Store>upsert_node` includes the caller. RED with any extraction-time-inline binding; GREEN only with the post-merge `resolve_call_site_contexts()` pass. ✅ Implemented: `resolve_call_site_contexts()` is a post-merge pass (runs after all files are indexed); `record_call_site` captures context at extraction time into a side table; the post-pass resolves against the complete store regardless of file indexing order. `store/tests.rs:2259` covers the partial-resolution case.
- [x] **AC-23 (Part A.2 — graph-theory queries on the real-symbol induced subgraph).** The 19 graph-theory queries (centrality / cycle / SCC / WCC / k-core / dependency-layers / articulation / bridges / biconnected / topo-sort / hubs / leaves / isolated / most-connected / singly-referenced) exclude `NodeKind::Unresolved` phantoms as **nodes** and as **edge endpoints**, routed through the new single-source-of-truth `Store::symbol_universe()`; centrality normalization denominators count `|real symbols|`. Each is RED-tested against a phantom-free twin oracle. ✅ Implemented: `symbol_universe_*` (3 tests), `{leaf_symbols,isolated_symbols,singly_referenced,hub_symbols,most_connected}_excludes_phantoms*`, `{k_core,dependency_layers,nodes_in_cycles,cycle_members,scc_groups,strongly_connected_components,weakly_connected_components,topological_sort,articulation_points,bridge_edges,biconnected_components}_excludes_phantoms`, `{betweenness,closeness}_centrality_excludes_phantoms_and_normalizes_over_real_n`, `harmonic_centrality_stats_excludes_phantoms_and_uses_real_n`, `page_rank_universe_equals_symbol_universe` (all `crates/mycelium-core/src/store/tests.rs`). Documented in [ADR-0012](../docs/adr/0012-graph-query-real-symbol-induced-subgraph.md).
- [x] **AC-22 (Part B cross-language: Python + TypeScript — Phase 2c).** With local-ctor binding captures added to the Python and TypeScript packs, a multi-type method (`upsert_node` on two classes) bound via `s = Store()` (Python) / `const s = new Store()` (TypeScript) resolves `s.upsert_node()` to `…>Store>upsert_node` and **not** `…>Trunk>upsert_node` (`extractor_{python,typescript}_receiver_type_binds_multi_match_method_f5`); a same-name binding rebound to a conflicting type **declines** (`extractor_{python,typescript}_shadowed_binding_declines_no_misbind`, the TS case exercising the `assignment_expression` reassignment form); a binding **reassigned to a non-constructor declines** rather than trusting the stale type under dynamic/structural typing (`extractor_{python,typescript}_rebind_to_non_ctor_declines_no_misbind`, via the `@binding.rebind` count check — Codex P1 #647); and a binding in one function never leaks into another (`extractor_{python,typescript}_receiver_inference_no_cross_function_leak`). Packs gain the binding/rebind captures (Charter §4); the core adds one language-agnostic rebind-count drop pass (inert for packs that emit no `@binding.rebind`, e.g. Rust today).

- [x] **AC-24 (Part B bare implicit-self method disambiguation — Java / C# / C++ / Go / Ruby).** A **bare** implicit-receiver self-method call (`void run() { speak(); }` with no `this.`/explicit receiver) where the method name is defined on **more than one** type previously left an unresolved bare stub, so `get-callers <EnclosingType>>speak` returned a confident-but-wrong **empty** set (Python & Rust were immune — they require `self.`/explicit receiver). The extractor now records a synthetic `ReceiverContext { receiver: "self", self_type: Some(<innermost enclosing type>) }` for an unresolved bare method call inside a class/type body, so rule (a) of `infer_receiver_type` returns the enclosing type and the post-merge `resolve_call_site_contexts()` pass binds the unique `<EnclosingType>>speak` edge (`extractor_{java,csharp,cpp,go,ruby}_bare_self_method_binds_multi_match`). The enclosing type is found lexically (Java/C#/C++/Ruby via `enclosing_self_type`/`enclosing_class_chain`) or via the Go method receiver (`go_receiver_type` on the enclosing `method_declaration`). Conservative by construction: a bare call with no enclosing type (a free function) records no self-context and resolves unchanged (`extractor_java_bare_free_function_still_resolves_no_self_context`); a bare call whose enclosing type does **not** define the method declines rather than mis-binding to another type's same-named method (`extractor_java_bare_self_method_not_on_enclosing_type_declines`). Side effect (core-only): Ruby caller attribution for a call inside a method now lands at `Class>method` (a Ruby-scoped `class`/`module` walk gated on a Ruby `method`/`singleton_method` ancestor in `enclosing_function_path`), retiring the prior flat-`method` known limitation. **Deferred (by design):** Ruby bare calls written **without** parentheses parse as plain `(identifier)` nodes indistinguishable from local-variable reads; matching them would manufacture phantom call edges, so they remain unresolved — only the parenthesised `speak()` form (an unambiguous `call`) is fixed. No pack files changed (core-only fix; pack parity preserved).
