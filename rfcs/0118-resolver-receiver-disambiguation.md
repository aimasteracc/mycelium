...
- [x] **AC-23 (Part B Rust rebind invalidation — closes the `let`-shadow gap).** The language-agnostic rebind-count drop pass (AC-22) was inert for Rust because the Rust pack emitted no `@binding.rebind`. With `@binding.rebind` added to every Rust `let_declaration`/`assignment_expression` target, a local first bound to a Title-case ctor then **shadowed by a non-constructor** RHS (`let s = Store::new(); let s = compute(); s.m();`, where the bare `compute()` call is invisible to the constructor-binding query) now **declines** instead of mis-binding `s.m()` to the stale `Store>m` — `let`-shadowing to a different type is legal Rust, so the hazard was real (`extractor_rust_shadow_to_non_ctor_declines_no_misbind`). Rust-only pack addition (Charter §4); core unchanged; the existing Rust receiver tests (`_binds_multi_match_method_f5`, `_shadowed_binding_declines_no_misbind`, `_receiver_inference_no_cross_function_leak`) stay green.
