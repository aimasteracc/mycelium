...
- **Part A (de-noise the symbol universe).** Unresolved callees today are minted as kind-less placeholder nodes (`store.upsert_node(...)` at `extractor/mod.rs:393` and `:401-402`). They carry a real `>`-qualified path, so `all_symbols` and `page_rank` count them as real code. They give those queries phantom members and phantom rank mass. Part A gives unresolved callees a dedicated `NodeKind::Unresolved` marker so the **two** genuinely-leaking queries exclude them by kind, not by the fragile `p.contains('>')` string heuristic.
- **Part B (bind method calls to definitions).** A method like `upsert_node` is defined on many types, so the simple resolver's `if matches.len() == 1` gate (`store/mod.rs:1209`) refuses to bind it and `get-callers` returns 0. Part B adds a **static receiver-type inference** step that disambiguates the multi-match case — but performs it **at extraction time**, where the per-call-site receiver identifier is still in scope, **not** in the shared-stub post-pass (which is architecturally incapable of per-call-site disambiguation).
...
The fix: per-call-site receiver type is only recoverable **at extraction time** (`extractor/mod.rs:324-407`), where the receiver identifier and the local/param/field/import context for *that* call site are in hand. RFC-0118 therefore performs disambiguation there, binding the `Calls` edge directly to `…>T>method`, and leaves the post-pass strictly as the conservative fallback.
